...
load_icode(int fd, int argc, char **kargv) {
    ...
    struct Page *page;
    //(3.1) read raw data content in file and resolve elfhdr
    struct elfhdr elfh, *elf = &elfh;
    if (0 != load_icode_read(fd, elf, sizeof(struct elfhdr), 0)) {
        goto bad_pgdir_cleanup_mm;
    }
    //(3.2) read raw data content in file and resolve proghdr based on info in elfhdr
    ...
    uint32_t phnum, vm_flags, perm;
    struct proghdr progh, *ph = &progh;
    for (phnum = 0; phnum < elf->e_phnum; ++phnum) {
        if (0 != load_icode_read(fd, ph, sizeof(struct proghdr), elf->e_phoff + phnum * sizeof(struct proghdr))) {
            goto bad_pgdir_cleanup_mm;
        }
    ...
    //(3.3) call mm_map to build vma related to TEXT/DATA
        ...
        unsigned char *offset = ph->p_offset;
        size_t off, size;
        ...
        while (start < end) {
            ...
            if (0 != load_icode_read(fd, page2kva(page) + off, size, offset)) {
                goto bad_pgdir_cleanup_mm;
            }
